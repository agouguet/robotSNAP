            ...
            if (ShouldPublishMessage)
            {
                ...
                if (m_GlobalFrameIds.Count > 0)
                {
                    var tfRootToGlobal = new TransformStampedMsg(
                        new HeaderMsg(Supervisor.instance.clock.Now(), m_GlobalFrameIds.Last()),
                ...
                for (var i = 1; i < m_GlobalFrameIds.Count; ++i)
                {
                    var tfGlobalToGlobal = new TransformStampedMsg(
                        new HeaderMsg(Supervisor.instance.clock.Now(), m_GlobalFrameIds[i - 1]),
                        m_GlobalFrameIds[i],
                ...
                for (var i = 0; i < tf_elements.Count; ++i)
                {
                    tfMessageList.Add(
                        ...
                        new RosMessageTypes.Std.HeaderMsg(Supervisor.instance.clock.Now(), tf_elements[i].transform.parent.gameObject.name),
